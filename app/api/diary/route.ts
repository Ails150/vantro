...
export async function POST(request: Request) {
  ...
  if (aiAlertType && aiAlertType !== "none") {
    ...
    await service.from("alerts").insert({
      company_id: resolvedCompanyId, job_id: jobId, user_id: resolvedUserId,
      message: (aiAlertType === "blocker" ? "BLOCKER" : "ISSUE") + " - " + (aiSummary || entryText.slice(0, 100)),
      alert_type: aiAlertType, is_read: false, status: "open"
    })
    const { data: admins } = await service.from("users").select("push_token").eq("company_id", resolvedCompanyId).in("role", ["admin", "foreman"]).not("push_token", "is", null)
      ...
      if (tokens.length > 0) {
        await fetch("https://exp.host/--/api/v2/push/send", {
          ...
          body: JSON.stringify(tokens.map((token: string) => ({
            ...
            data: { type: "diary_alert", jobId, alertType: aiAlertType }, channelId: "vantro",
          })))
        }).catch(() => {})
      }
    }
    ...
    if (recipients && recipients.length > 0 && process.env.RESEND_API_KEY) {
      for (const recipient of recipients.filter((r: any) => r.email)) {
        await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: "Vantro Alerts <alerts@getvantro.com>", to: recipient.email,
            subject: (aiAlertType === "blocker" ? "BLOCKER" : "ISSUE") + " - " + (job?.name || "Job"),
            html: "<div style=\"font-family:sans-serif\"><h2>Vantro Alert</h2><p><strong>" + (aiAlertType === "blocker" ? "BLOCKER" : "ISSUE") + "</strong></p><p>Job: " + (job?.name || "Unknown") + "</p><p>By: " + (alertUser?.name || "Unknown") + "</p><p>" + (aiSummary || entryText) + "</p>" + (photoUrls && photoUrls.length > 0 ? "<p>" + photoUrls.map((u: string) => "<img src='" + u + "' style='max-width:200px;margin:4px;border-radius:8px;'>").join("") + "</p>" : "") + "<a href=\"https://app.getvantro.com/admin\">View Dashboard</a></div>"
          })
        }).catch(() => {})
      }
    }
...
export async function GET(request: Request) {
  ...
  const { data: entries } = await service
    .from('diary_entries')
    .select('id, entry_text, ai_alert_type, ai_summary, reply, replied_at, created_at, user_id, photo_urls')
    .eq('job_id', jobId)
