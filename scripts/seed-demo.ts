...
/** A real, tiny, valid PDF.
 *
 *  The upload route and the phone both refuse anything that is not
 *  application/pdf, so demo data that could not have travelled the real path
 *  would be misleading. Hand-assembled rather than pulling in a library: it is
 *  one page of text, and pdf-lib is a heavy dependency for a seed script. */
function demoPdf(title: string, jobName: string): Buffer {
  const esc = (t: string) => t.replace(/([\\()])/g, "\\$1")
  const lines = [
    esc(title),
    esc(jobName),
    "DEMO DATA - not a real method statement.",
    "Generated by scripts/seed-demo.ts.",
  ]
  const text =
    "BT /F1 14 Tf 60 760 Td 20 TL " +
    lines.map((l, i) => (i === 0 ? "" : "T* ") + "(" + l + ") Tj").join(" ") +
    " ET"
  const objects = [
...
export async function seedDemo(
  ...
  // --- RAMS ----------------------------------------------------------------
  // Two versions on the Cambridge job, because a revision is the thing that
  // actually catches people out: it retires the old version and puts the whole
  // crew back to unsigned. The other two jobs get one version each, fully
  // signed, so the contrast is visible in the pack.
  let ramsVersions = 0
  let ramsSignatures = 0
  let ramsOutstanding = 0

  const ramsPlan = [
    { job: jobs[0], title: "Curtain walling installation - RAMS rev B", revision: null as string | null, daysAgo: 40, signers: "all" as const },
    {
      job: jobs[0],
      title: "Curtain walling installation - RAMS rev C",
      revision: "Revised lifting plan for the west elevation after the crane position moved.",
      daysAgo: 4,
      signers: "most" as const,
    },
    { job: jobs[1], title: "Window replacement - RAMS rev A", revision: null, daysAgo: 38, signers: "all" as const },
    { job: jobs[2], title: "Glazed atrium - RAMS rev A", revision: null, daysAgo: 37, signers: "all" as const },
  ]

  for (const plan of ramsPlan) {
    const createdAt = atLocal(addDays(today, -plan.daysAgo), 8, 10)
    const pdf = demoPdf(plan.title, plan.job.name)
    const sha = sha256Hex(pdf)

    let documentUrl = "local://demo/" + sha.slice(0, 16) + ".pdf"
    let documentPath = "demo/" + sha.slice(0, 16) + ".pdf"
    const client = r2Client()
    const publicBase = (process.env.CLOUDFLARE_R2_PUBLIC_URL || "").replace(/\/$/, "")
    if (client && publicBase) {
      documentPath = "rams/" + companyId + "/" + plan.job.id + "/" + sha.slice(0, 16) + ".pdf"
      await client.send(new PutObjectCommand({
        Bucket: R2_BUCKET, Key: documentPath, Body: pdf, ContentType: "application/pdf",
        Metadata: { "demo-seed": "northbridge", "company-id": companyId },
      }))
      await recordFileHash({ companyId, storagePath: documentPath, sha256: sha, hashedBy: adminId })
      documentUrl = publicBase + "/" + documentPath
    }

    // Retire the version in force first: the partial unique index allows
    // exactly one live version per job, and the upload route observes the same
    // ordering for the same reason.
    const { data: prior } = await service
      .from("rams_documents")
      .select("id, version")
      .eq("job_id", plan.job.id)
      .is("superseded_at", null)
      .maybeSingle()
    if (prior) {
      await service.from("rams_documents")
        .update({ superseded_at: iso(createdAt) })
        .eq("id", prior.id)
    }
    ...
    const { data: doc, error: docErr } = await service
      ...
      .single()
    if (docErr || !doc) {
      warnings.push('RAMS "' + plan.title + '" failed: ' + docErr?.message)
      continue
    }
    if (prior) {
      await service.from("rams_documents").update({ superseded_by: doc.id }).eq("id", prior.id)
    }
    ramsVersions++

    const attendees = crew[plan.job.id]
    const signers = plan.signers === "all" ? attendees : attendees.slice(0, Math.max(1, attendees.length - 2))
    for (const [i, userId] of signers.entries()) {
      const signedAt = new Date(createdAt.getTime() + (12 + i * 4) * 60000)
      const { error: sigErr } = await service.from("rams_signatures").insert({
        company_id: companyId,
        rams_id: doc.id,
        user_id: userId,
        signature_svg: demoSignature(100 + i),
        signed_at: iso(signedAt),
        // A plausible spread: some read it properly, some clearly did not.
        read_seconds: Math.floor(20 + r() * 260),
        lat: plan.job.lat + (r() - 0.5) * 0.0006,
        lng: plan.job.lng + (r() - 0.5) * 0.0006,
        accuracy_metres: Math.floor(r() * 15) + 5,
        device_info: "Demo seed",
      })
      if (sigErr) warnings.push("RAMS signature failed: " + sigErr.message)
      else ramsSignatures++
    }
  }

  // Only the versions still in force count towards the outstanding figure: a
  // superseded version's unsigned names are history, not a live gap.
  {
    ...
    for (const d of live || []) {
      const { count } = await service
        ...
        .eq("rams_id", d.id)
      ramsOutstanding += Math.max(0, (crew[d.job_id]?.length || 0) - (count || 0))
    }
  }
