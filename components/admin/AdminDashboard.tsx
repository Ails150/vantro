    ...
    <div className="min-h-screen bg-[#0a1520] text-white">
      ...
      <div className="bg-[#0f1e30] border-b border-white/8 px-8 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleSignOut} className="text-sm text-[#7a9ab8] hover:text-white transition-colors border border-white/10 rounded-full px-4 py-2">Sign out</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 px-8 py-6">
        {[
          ...
          <div key={s.label} className="bg-[#0f1e30] border border-white/8 rounded-2xl p-6">
            <div className="text-[#7a9ab8] text-sm font-medium mb-3">{s.label}</div>
      ...
      <div className="flex border-b border-white/8 px-8">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-5 py-4 text-base font-medium border-b-2 transition-colors ${activeTab === tab.id ? "border-[#00d4a0] text-white" : "border-transparent text-[#7a9ab8] hover:text-white"}`}>
            {tab.label}
      ...
      <div className="px-8 py-8 max-w-6xl">
              ...
              : signins.map((s: any) => (
                <div key={s.id} className="flex items-center gap-4 px-6 py-4 border-b border-white/5 last:border-0">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-[#7a9ab8]">In at {new Date(s.signed_in_at).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</div>
                  </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className="text-sm text-[#7a9ab8]">on {qa.jobs?.name}</span>
                    </div>
                    {qa.notes && <div className="text-sm text-[#7a9ab8] mt-1">Note: {qa.notes}</div>}
                  </div>
        ...
        {activeTab === "jobs" && (
          <div className="space-y-5">
            <div className="flex justify-end">
              <button onClick={() => { setShowAddJob(true); setFormError("") }} className="bg-[#00d4a0] hover:bg-[#00a87e] text-[#0a1520] font-bold rounded-xl px-5 py-3 text-sm transition-colors">+ Add job</button>
            </div>
            {showAddJob && (
              <div className="bg-[#0f1e30] border border-[#00d4a0]/30 rounded-2xl p-6 space-y-4">
                ...
                <div className="flex gap-3">
                  <button onClick={addJob} disabled={saving} className="bg-[#00d4a0] hover:bg-[#00a87e] disabled:opacity-50 text-[#0a1520] font-bold rounded-xl px-5 py-3 text-sm">{saving ? "Saving..." : "Save job"}</button>
                  <button onClick={() => setShowAddJob(false)} className="bg-white/5 text-[#7a9ab8] rounded-xl px-5 py-3 text-sm">Cancel</button>
                </div>
              </div>
            )}
            <div className="bg-[#0f1e30] border border-white/8 rounded-2xl overflow-hidden">
        ...
        {activeTab === "team" && (
          <div className="space-y-5">
            <div className="flex justify-end">
              <button onClick={() => { setShowAddMember(true); setFormError("") }} className="bg-[#00d4a0] hover:bg-[#00a87e] text-[#0a1520] font-bold rounded-xl px-5 py-3 text-sm transition-colors">+ Add member</button>
            </div>
            {showAddMember && (
              <div className="bg-[#0f1e30] border border-[#00d4a0]/30 rounded-2xl p-6 space-y-4">
                ...
                {formError && <p className="text-sm text-red-400">{formError}</p>}
                <div className="flex gap-3">
                  <button onClick={addMember} disabled={saving} className="bg-[#00d4a0] hover:bg-[#00a87e] disabled:opacity-50 text-[#0a1520] font-bold rounded-xl px-5 py-3 text-sm">{saving ? "Saving..." : "Save member"}</button>
                  <button onClick={() => setShowAddMember(false)} className="bg-white/5 text-[#7a9ab8] rounded-xl px-5 py-3 text-sm">Cancel</button>
                </div>
              </div>
            )}
            <div className="bg-[#0f1e30] border border-white/8 rounded-2xl overflow-hidden">
              ...
              : teamMembers.map((m: any) => (
                <div key={m.id} className="flex items-center gap-4 px-6 py-5 border-b border-white/5 last:border-0">
                  ...
                  <span className="text-sm bg-white/5 text-[#7a9ab8] px-3 py-1 rounded-full capitalize font-medium">{m.role}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                return (
                  <div key={m.id} className="flex items-center gap-4 px-6 py-5 border-b border-white/5 last:border-0">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-[#7a9ab8]">today</div>
                    </div>
                  </div>
                )
              })}
