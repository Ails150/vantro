...
export default function AuditTab({ jobs }: Props) {
  ...
  async function generate() {
    ...
    setLoading(true); setError(""); setPreview(null); setShareLink("")
    try {
      ...
      setPreview(data)
      // Also load any existing share links for this job
...
<div class="meta">
  ...
  <strong>Generated:</strong> ${new Date(preview.generated || Date.now()).toLocaleString("en-GB")}<br>
  <strong>Report Reference:</strong> ${refId}<br>
  <strong>Report produced by:</strong> Vantro &mdash; getvantro.com
</div>
...
${preview.signins.map((s: any) => {
  ...
  const hrs = outT ? ((outT.getTime() - inT.getTime()) / 3600000).toFixed(1) : "â€”"
  return `<tr><td>${s.users?.name || "Unknown"}</td><td>${inT.toLocaleString("en-GB")}</td><td>${outT ? outT.toLocaleString("en-GB") : '<span class="fail">Not signed out</span>'}</td><td>${s.distance_metres ?? "â€”"}m</td><td>${s.sign_out_distance_metres ?? "â€”"}m</td><td>${hrs}</td></tr>`
}).join("")}</table>`}
...
${preview.diary.map((e: any) => `<tr><td>${new Date(e.created_at).toLocaleString("en-GB")}</td><td>${e.users?.name || "Unknown"}</td><td>${e.entry_text || ""}</td><td class="${e.ai_alert_type}">${e.ai_alert_type !== "none" ? (e.ai_summary || e.ai_alert_type) : "â€”"}</td><td>${e.photo_urls && e.photo_urls.length > 0 ? e.photo_urls.map((u: string) => `<img src="${u}">`).join("") : "â€”"}</td></tr>`).join("")}</table>`}
...
${preview.qa.map((q: any) => `<tr><td>${new Date(q.created_at).toLocaleString("en-GB")}</td><td>${q.users?.name || "Unknown"}</td><td>${q.checklist_items?.label || "â€”"}</td><td class="${q.result === "pass" ? "pass" : "fail"}">${q.result?.toUpperCase() || "â€”"}</td><td>${q.note || "â€”"}</td><td>${q.photo_url ? `<img src="${q.photo_url}">` : "â€”"}</td></tr>`).join("")}</table>`}
...
<div class="footer">
  ...
  CNNCTD Ltd &mdash; Vantro is a product of CNNCTD Ltd (NI695071)
</div>
</body>
</html>`
    ...
    a.href = url
    a.download = `Vantro-Audit-${preview.job.name.replace(/\s+/g, "-")}-${new Date().toISOString().slice(0, 10)}.html`
    a.click()
  ...
  return (
    <div className="space-y-5">
      <div className={card + " p-6"}>
        <h2 className="text-xl font-semibold text-gray-900 mb-1">Audit & Dispute Report</h2>
        <p className="text-sm text-gray-500 mb-5">
          Generate a full evidence pack for any job â€” diary entries, photos, GPS sign-ins, QA responses.
        </p>
        ...
        <div className={card + " p-6"}>
          <div className="flex items-start justify-between mb-5 gap-3">
            <div className="flex-1">
              <div className="font-semibold text-gray-900">{preview.job.name}</div>
              <div className="text-sm text-gray-500">{preview.job.address}</div>
            </div>
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                ...
                className="px-4 py-2 bg-white border border-teal-300 text-teal-700 hover:bg-teal-50 rounded-lg text-sm font-semibold disabled:opacity-50"
              >
                {creatingLink ? "Creatingâ€¦" : "ðŸ”— Share with client"}
              </button>
            </div>
          </div>
          ...
          {shareLink && (
            <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-5">
              <div className="text-sm font-medium text-teal-900 mb-2">Share link created â€” expires in 30 days</div>
              ...
              <p className="text-xs text-teal-700 mt-2">
                Send this to your client. They'll see the report without needing to log in.
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {[
              { label: "Sign-ins", value: preview.signins.length, color: "text-teal-600" },
              { label: "Diary entries", value: preview.diary.length, color: "text-teal-600" },
              { label: "Photos", value: preview.diary.reduce((n: number, e: any) => n + (e.photo_urls?.length || 0), 0), color: "text-teal-600" },
              { label: "QA responses", value: preview.qa.length, color: "text-teal-600" },
              { label: "GPS Points", value: preview.breadcrumbs?.length || 0, color: "text-teal-600" },
              { label: "Blockers/Issues", value: preview.diary.filter((e: any) => e.ai_alert_type && e.ai_alert_type !== "none").length, color: "text-red-600" },
            ].map((stat) => (
      ...
      {activeShares.length > 0 && (
        <div className={card}>
          <div className="px-6 py-4 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-500 mt-0.5">Links you've sent to clients for this job.</p>
          </div>
          {activeShares.map((s) => (
            <div key={s.id} className="border-b border-gray-50 last:border-0 px-6 py-4 flex items-center gap-4">
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Created {new Date(s.created_at).toLocaleDateString("en-GB")} Â·
                  Expires {new Date(s.expires_at).toLocaleDateString("en-GB")} Â·
                  Viewed {s.view_count || 0} time{s.view_count === 1 ? "" : "s"}
                </div>
              </div>
              <button
