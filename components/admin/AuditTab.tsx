...
${preview.diary.map((e: any) => `<tr><td>${new Date(e.created_at).toLocaleString("en-GB")}</td><td>${e.users?.name || "Unknown"}</td><td>${e.entry_text || ""}</td><td class="${e.ai_alert_type}">${e.ai_alert_type !== "none" ? (e.ai_summary || e.ai_alert_type) : "—"}</td><td>${e.photo_urls && e.photo_urls.length > 0 ? e.photo_urls.map((u: string) => `<img src="${u}">`).join("") : "—"}</td></tr>`).join("")}</table>`}
...
${preview.qa.map((q: any) => `<tr><td>${new Date(q.created_at).toLocaleString("en-GB")}</td><td>${q.users?.name || "Unknown"}</td><td>${q.checklist_items?.label || "—"}</td><td class="${q.result === "pass" ? "pass" : "fail"}">${q.result?.toUpperCase() || "—"}</td><td>${q.note || "—"}</td><td>${q.photo_url ? `<img src="${q.photo_url}">` : "—"}</td></tr>`).join("")}</table>`}
